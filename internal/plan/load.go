...
// by the same path.
func Load(path string) (*tfjson.Plan, Status, error) {
	b, err := os.ReadFile(path)
	if err != nil {
		return nil, Status{}, fmt.Errorf("cannot read %s: %w", path, err)
	}
	return parse(b, path)
}
...
func Read(r io.Reader, name string) (*tfjson.Plan, Status, error) {
	b, err := io.ReadAll(r)
	if err != nil {
		return nil, Status{}, fmt.Errorf("cannot read %s: %w", name, err)
	}
	return parse(b, name)
}

func parse(b []byte, name string) (*tfjson.Plan, Status, error) {
	...
	if len(bytes.TrimSpace(b)) == 0 {
		return nil, Status{}, fmt.Errorf("%s is empty: expected plan JSON from: terraform show -json tfplan", name)
	}
	...
	if err := json.Unmarshal(b, &probe); err != nil {
		return nil, Status{}, fmt.Errorf("%s is not valid JSON: %w", name, err)
	}

	if probe.FormatVersion == "" {
		return nil, Status{}, fmt.Errorf("%s does not look like a Terraform plan: no format_version. Generate one with: terraform show -json tfplan > plan.json", name)
	}
